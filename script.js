...
function init() {
    if (storedTodos !== null) {
        /* Here i clone get all the information from the localStorage and set to todos!*/
        todos = storedTodos;
        for (i = 0; i < time.length; i++) {
            /* Where the for work i will send id to build page and show the text at textearea */
            createPage(i);
            showTextArea(i);
        ...
        /* If storedTodos is empty we create a new array to save all information from the localStorage*/
        for (i = 0; i < time.length; i++) {
            var newArray = i + 9;
...
/* Here is where i create all elementes and set at the index */
function createPage(i) {
    /* I create all elementes and set class, id, name or icons here */
    var divRow = $('<div>');
    divRow.addClass('row');
    var divLabel = $('<div>');
